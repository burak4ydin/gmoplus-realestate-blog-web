...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  try {
    ...
    return {
      title: `${name} — GMOPlus Real Estate Blog`,
      description: `${name} kategorisindeki gayrimenkul makaleleri.`,
    };
  } catch {
    return { title: "Kategori — GMOPlus Real Estate Blog" };
  }
}
    ...
    <div className="max-w-5xl mx-auto px-4 py-10">
      ...
      <header className="mb-8">
        <div className="inline-flex items-center gap-2 mb-3">
          <span className="text-xs font-semibold text-orange-600 bg-orange-50 px-2.5 py-1 rounded-full">KATEGORİ</span>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 mb-10">
          {data.items.map((post: any) => (
            <article key={post.id} className="group bg-white rounded-xl border border-gray-100 overflow-hidden hover:shadow-md transition-shadow">
              ...
              <div className="p-5">
                {post.category && (
                  <span className="text-xs font-semibold text-orange-600 bg-orange-50 px-2 py-0.5 rounded mb-2 inline-block">
                    {post.category.name}
                  </span>
                )}
                <a href={`/${post.slug}`}>
